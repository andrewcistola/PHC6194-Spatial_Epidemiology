...
df_Q4 = subset(df_arrest, Murder >= 10 & UrbanPopG >= 2) # Subset data frame by multiple column values with boolean operators
...
df_arrest = data.frame(USArrests) # Save object as data frame
df_arrest$UrbanPopG = cut(df_arrest$UrbanPop, breaks = quantile(df_arrest$UrbanPop, probs = c(0,0.25,0.5,0.75,1)), include.lowest = T) # Create column of quantiles based on column
linear_Q7 = lm(Murder ~ Assault + factor(UrbanPopG) + Rape, data = df_arrest) # Create multiple linear regression model
summary(linear_Q7) # Print summary of model 
...
p_Q8 = ggplot(data = df_arrest, aes(x = Assault, y = Murder)) + geom_point(aes(color = UrbanPopG, size = Rape, alpha = .9)) + facet_wrap(~UrbanPopG) # Create multi-panel plot based on variable with point size and transparency options
ggsave('/home/drewc/GitHub/_colab/PHC6194/_student/_fig/A1_Q8_graph.jpeg', plot = p_Q8)
...
p_Q9 = ggplot(data = df_arrest, aes(x = Assault, y = Murder)) + geom_point(aes(color = UrbanPopG, size = Rape, alpha = .9)) + stat_smooth() + facet_wrap(~UrbanPopG) # Create multi-panel plot based on variable with point size and transparency options
ggsave('/home/drewc/GitHub/_colab/PHC6194/_student/_fig/A1_Q9_graph.jpeg', plot = p_Q8)
